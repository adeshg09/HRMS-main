...
/* Constants */
const addUserPath = PAGE_COMPANY_DASHBOARD.users.create.relativePath;
const editUserPath = PAGE_COMPANY_DASHBOARD.users.edit.relativePath;
...
const initialStateShowActivityDialog = {
  open: false,
  title: <>Update</>,
  description: <>Are you sure you want to show activity to this user?</>,
  userId: 0,
  showActivity: 0
};
...
const ManageUser = (): JSX.Element => {
  ...
  const handleShowActivityDialog = (
    ...
    const popuptext = `${
      showActivity === true
        ? 'Are you sure you want to hide activity for this user?'
        : 'Are you sure you want to show activity for this user?'
    }`;
    const popupdesc = (
      <>
        {popuptext} - <b>"{userName}"</b>?
  ...
  const handleUpdateShowActivityStatus = async (
    ...
    try {
      ...
      if (response && response.status.response_code === 200) {
        showSnackbar(
          toastMessages.success.adminDashboard.userActivityStatusUpdated,
          'success'
        );
        setRows(
          updateNestedItemFromArray(
